...
/// Класс для хранения результатов проверки утечки DNS
class DnsLeakResult {
  final bool hasLeaks;
  final List<String> leakDetails;
  
  DnsLeakResult(this.hasLeaks, this.leakDetails);
}
...
class ConnectionHealthMonitor {
  ...
  Future<HealthStatus> checkHealth() async {
    ...
    try {
      ...
      if (!hasInternet) {
        ...
        final DnsLeakResult leakCheck = await _checkForDnsLeaks();
        ...
        if (avgLatency == -1 || validLatencies.isEmpty) {
          status = HealthStatus(
            ...
            leakDetails: leakCheck.leakDetails,
          );
  ...
  // Check for DNS leaks
  Future<DnsLeakResult> _checkForDnsLeaks() async {
    if (!_connectionManager.isConnected) {
      return DnsLeakResult(false, []);
    }
    ...
    try {
      ...
      try {
        ...
        return DnsLeakResult(unexpectedServers.isNotEmpty, unexpectedServers);
      } catch (e) {
        // Parsing error
...
// Хелпер для показа уведомлений о состоянии VPN
class VpnHealthNotificationHelper {
