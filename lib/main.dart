...
class _AppLayoutState extends State<AppLayout> with WindowListener, TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Создаем TabController с правильной длиной (5 вкладок)
  ...
  void _forceUpdatePages() {
    ...
    setState(() {
      ...
      _homePage = HomePage(selectedServer: AppGlobals.selectedServer);
      _serversPage = ServersPage(initialSelectedServer: AppGlobals.selectedServer);
      // Страницы _logsPage, _routingPage и _settingsPage не нуждаются в обновлении
    });
  }
  ...
  void _handleTabSelection() {
    if (_tabController.indexIsChanging) {
      // Добавляем проверку на допустимые индексы
      if (_tabController.index >= 0 && _tabController.index < _pages.length) {
  ...
  void goToHomeWithServer(VpnConfig server) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        // Проверка индекса перед анимацией
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 5, // Обновлено до 5 вкладок
      child: Scaffold(
        body: Column(
          children: [
