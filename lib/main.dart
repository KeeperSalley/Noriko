...
class _AppLayoutState extends State<AppLayout> with WindowListener, TickerProviderStateMixin {
  ...
  // Ключ для доступа к состоянию из дочерних виджетов
  final GlobalKey<_AppLayoutState> _appLayoutKey = GlobalKey<_AppLayoutState>();
  ...
  // Метод для переключения на страницу серверов
  void goToServersPage() {
    setState(() {
      _serversPage = const ServersPage();
      _tabController.animateTo(1); // Переключаемся на вторую вкладку (Серверы)
    });
  }
...
// Глобальная функция для доступа к методам AppLayoutState из любого места
void goToHomeWithServer(BuildContext context, VpnConfig server) {
  AppGlobals.selectedServer = server;
  
  // Используем TabController из состояния _AppLayoutState
  final state = context.findAncestorStateOfType<_AppLayoutState>();
  if (state != null) {
    state.goToHomeWithServer(server);
  } else {
    // Если состояние не найдено, используем DefaultTabController
...
// Глобальная функция для перехода на страницу серверов
