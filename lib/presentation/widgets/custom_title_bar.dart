      ...
      child: Row(
        children: [
          ...
          _buildWindowButton(
            icon: Icons.close,
            onPressed: () async {
              // Проверяем, активна ли настройка сворачивания в трей
              bool minimizeToTray = await SystemTrayService.isMinimizeToTrayEnabled();
              
              if (minimizeToTray) {
                // Если настройка активна, просто сворачиваем в трей без диалога
                ...
                // Если настройка неактивна, просто вызываем штатный процесс закрытия с диалогом
